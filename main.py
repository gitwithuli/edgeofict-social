...
from core.automation import run_daily_quote_publish, run_daily_stoic_publish
...
@click.group()
def cli():
    """EdgeOfICT Social Media Automation CLI"""
    pass
...
def auto_quote(run_hour: int | None, dry: bool, force: bool):
    """Publish the daily evening quote post."""
    result = run_daily_quote_publish(run_hour=run_hour, dry_run=dry, force=force)
...
def status():
    ...
    console.print(Panel(
        "[cyan]EdgeOfICT Social Media Agent[/cyan]\n"
        "[dim]Track Your Trading Edge[/dim]",
        box=box.ROUNDED
    ))
...
def init():
    ...
    console.print("[cyan]Initializing EdgeOfICT Social...[/cyan]\n")
